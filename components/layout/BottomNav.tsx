...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-slate-950/95 backdrop-blur-lg border-t border-[#E6E4DF] dark:border-[#2D2A26] px-2 py-1 max-w-md mx-auto">
          ...
          if (item.isAction) {
            return (
              <button 
                ...
                className="relative -top-3.5 bg-[#1F6E5C] hover:bg-[#2B9B82] text-white w-11 h-11 rounded-full flex items-center justify-center shadow-md transition-all active:scale-90 hover:scale-105 border-4 border-[#FAF9F6] dark:border-[#121210]"
                title="Add Expense"
              >
          ...
          return (
            <Link 
              ...
              className={`flex flex-col items-center justify-center py-0.5 px-3 rounded-xl transition-all duration-200 gap-0.5 ${
                isActive 
                  ? 'text-[#1F6E5C] dark:text-[#2B9B82] font-extrabold scale-105' 
